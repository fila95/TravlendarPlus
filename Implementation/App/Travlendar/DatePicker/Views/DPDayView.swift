...
public final class DPDayView: UIView {
    
    // MARK: - Properties
    private var index: Int!
    private var dayInfo: DPDay!
    weak private var weekView: DPWeekView!
    weak private var monthView: DPMonthView!
    ...
    private weak var selectionView: DPSelectionView?
    ...
    init(datePickerView: DPView, monthView: DPMonthView, weekView: DPWeekView, index: Int, dayInfo: DPDay) {
    ...
    private func setupLabelFont() {
        ...
        if preferredFont.fontName.isEmpty {
            ...
            if let customFont = UIFont(name: preferredFont.fontName, size: sizeOfFont) {
                ...
                print("custom font '\(preferredFont.fontName)' for dayLabel not available. DP will use system font instead")
                fontToUse = UIFont.systemFont(ofSize: sizeOfFont, weight: UIFont.Weight.regular)
            }
        }
    ...
    func select() {
        ...
        let selView = DPSelectionView(dayView: self, frame: self.bounds, isSemiSelected: false)
    ...
    private func semiSelect(animated: Bool) {
        ...
        if let selectionView = selectionView {
            ...
            let selView = DPSelectionView(dayView: self, frame: self.bounds, isSemiSelected: true)
            if animated {
    ...
    private func insertCircleViewAnimated(selectionView: DPSelectionView) {
